<script lang="ts">
  ...
  let { server = $bindable(), token = $bindable(), connected, loading, error, auxCode, auxBusy, guestMode, importing, syncMessage, onImportFiles, onClose, onReconnect, onDisconnect, onStartAux, onShowAux, onEndAux, onJoinAux }: {
...
<MobileSheet full title="Settings" {onClose}>
  <div class="native-settings">
    ...
    {#if !guestMode}
      ...
      </div><p>{syncMessage || "Choose a bundle ZIP to include its audio, playlists, and covers, or select MP3 files."}</p></section>
    {/if}
...
{#if joining}<MobileSheet title="Join Aux" onClose={() => joining = false}><form class="mobile-create-playlist" onsubmit={(event) => { event.preventDefault(); joining = false; onJoinAux(code.trim().toUpperCase()); }}><label>Aux code<input bind:value={code} placeholder="4-character code" maxlength="4" autocapitalize="characters" /></label><button class="ui-button primary" disabled={code.trim().length !== 4}>Join Aux</button></form></MobileSheet>{/if}
